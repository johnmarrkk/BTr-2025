<script>
  ...
  onMount(() => {
    fetchNumberQueue(); // Fetch initial data if needed

    const channel = supabase
      .channel("numberqueue")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "numberqueue" },
        (payload) => {
          numberqueue = [...numberqueue, payload.new]; // Ensure UI updates
        },
      )
      .subscribe();
      ...
      <div class="h-[650px] rounded-lg overflow-y-auto scrollbar-hide">
        <Table shadow class="!bg-white !text-black ">
          <TableHead class="!bg-white text-gray-700">
            <TableHeadCell class="!bg-yellow-400 !text-white text-right text-sm w-full"
